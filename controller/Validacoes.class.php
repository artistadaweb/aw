    ...
    public static function valNomes($val){
        if(strlen($val)>4){
            if(preg_match("/^([a-zA-Z0-9àèìòùÀÈÌÒÙáéíóúýÁÉÍÓÚÝâêîôûÂÊÎÔÛãñõÃÑÕäëïöüÿÄËÏÖÜŸçÇßØøÅåÆæœ\-\.\,]+\s?)*\s*$/",$val)){
                return array(true,"O valor do campo  está inválido.");
            }else{
                return array(false,"O valor do campo  está inválido.");
            }
        }else{
            return array(false,"O valor do campo está muito curto.".$val);
        }
    }
